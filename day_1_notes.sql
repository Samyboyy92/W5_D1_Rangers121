...
-- query for a first_name that equal Nick using the WHERE clause
SELECT first_name, last_name
FROM actor
...
-- Query for the first_name that is LIKE Nick
SELECT first_name, last_name, actor_id
FROM actor
...
-- using the WHERE clase with both wildcard and underscore
SELECT first_name, last_name, actor_id
FROM actor
...
-- Explore data from payments table with SELECT ALL query *
-- Good idea to grab all data from a new table so you know what your're working with
SELECT *
FROM payment;

-- Query for data that shows customers who paid
-- an ammount great than $3
...
-- Query for payments that were greater than or euqal to 5.99
SELECT customer_id, rental_id, amount
FROM payment
WHERE amount >= 5.99;
...
-- Query for data that shows customers who paid amount BETWEEN 2.00 and 7.99
...
-- Query to display the COUNT of amount paid
SELECT COUNT(amount)
FROM payment
WHERE amount > 5.99;
